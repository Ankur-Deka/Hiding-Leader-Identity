...
class Scenario(BaseScenario):
    def __init__(self, num_agents=3, dist_threshold=0.2, arena_size=1, identity_size=0, num_steps=100, diff_reward=True, same_color = False, random_leader_name=False, goal_at_top = False, hide_goal=False):
        ...
        # self.goal_loc = np.array([0,0.9], dtype = float)
        ...
        # self.traj_pts = [np.linspace(-self.arena_size, self.arena_size, 100), self.traj_func(np.linspace(-self.arena_size, self.arena_size, 100))]


    def traj_func(self, y):
        # given y coordinate(s), return the x coordinate(s)
        return self.traj_amp*np.sin(np.pi*y+self.traj_phase)
    ...
    def make_world(self):
        ...
        # add trajectory 
        self.traj_amp, self.traj_phase = 0.5,0
        world.traj_points = np.array([self.traj_func(np.linspace(-self.arena_size, self.arena_size, 100)), np.linspace(-self.arena_size, self.arena_size, 100)]).T
    ...
    def reset_world(self, world):
        ...
        self.traj_amp, self.traj_phase = np.random.uniform(low=[0.3,0], high=[0.9, 2*np.pi])
        world.traj_points = np.array([self.traj_func(np.linspace(-self.arena_size, self.arena_size, 100)), np.linspace(-self.arena_size, self.arena_size, 100)]).T
        self.start_loc[0] = world.traj_points[0,0]
        ...
        # if self.goal_at_top:
        #     self.goal_loc = np.random.uniform([-size, 0.9*size], [size,0.9*size])
        # else:
        #     self.goal_loc = np.random.uniform([-size, 0], [size,0.9*size])
        ...
        for agent in world.agents:
            ...
            agent.state.p_pos += np.random.uniform(-0.25*size, 0.25*size, size=2)#[-0.25*size, -size], [0.25*size,-size*0.9])
            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)
            agent.prev_dist = None

        # for i, landmark in enumerate(world.landmarks):
        #     landmark.state.p_pos = self.goal_loc
    ...
    def reward(self, agent, world):
        ...
        # dist = np.linalg.norm(agent.state.p_pos-self.goal_loc)
        # if self.diff_reward:
        ...
        x_pos, y_pos = agent.state.p_pos 
        x_traj = self.traj_func(y_pos)
        reward = y_pos - 5*np.linalg.norm(x_pos-x_traj)
    ...
    def observation(self, agent, world):
        ...
        nxt_pos = [[0,0]]
        if agent.is_leader:
            leader_x, leader_y = agent.state.p_pos
            nxt_y = leader_y+0.1*self.arena_size
            nxt_x = self.traj_func(nxt_y)

            nxt_pos = [[nxt_x-leader_x, nxt_y-leader_y]]
        # print('agent', float(agent.is_leader))
        default_obs = np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + nxt_pos)
        # print(default_obs)
    ...
    def info(self, agent, world):
        info = {'is_success': False, 'world_steps': world.steps,
