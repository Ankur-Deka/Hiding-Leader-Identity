...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 cam_range=1, num_steps = 128, video_format = 'webm', fps=10
                 ):

        self.world = world
        self.world.max_steps_episode = num_steps
        ...
        if video_format == 'mp4':
            self.fourcc = cv2.VideoWriter_fourcc(*'mp4v')#MP4V')
    ...
    def step(self, action_n):
        ...
        success_list = [agent_info['is_success'] for agent_info in info_n['n']]
        info_n['is_success'] = np.all(success_list)
        info_n['env_done'] = np.all(done_n)
        # if info_n['is_success'] or info_n['env_done']:
        #     print('is_success env', info_n['is_success'], 'env_done', info_n['env_done'])
    ...
    def _reset_render(self):
        ...
        self.render_texts = None
        self.render_texts_xforms = None
    ...
    def render(self, mode='human', attn=None):
        ...
        if self.render_geoms is None or True:
            ...
            # Trajectory points
            geom = rendering.make_polyline(self.world.traj_points)
            geom.set_color(1,0.5,0.5,0.5)
            geom.set_linewidth(3)
            xform = rendering.Transform()
            self.render_geoms.append(geom)
            self.render_geoms_xform.append(xform)
        ...
        for i in range(len(self.viewers)):
            ...
            #     self.render_geoms_xform[e].set_translation(*entity.state.p_pos)
                # if 'agent' in entity.name:
                #     self.render_texts_xforms[a] = entity.state.p_pos
                #     a += 1
